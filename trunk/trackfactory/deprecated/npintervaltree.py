...
def _init_pnode(node, id, start, end, value):
    node['id'] = id
    node['cleft_id'] = EMPTY
...
def insert(tbl, root_id, cur_id, start, end, value):
    if root_id == EMPTY:
...
def intersect(tbl, id, start, end):
    """
...
def left(tbl, id, position, n=1, max_dist=2500):       
    """
...
def right(tbl, id, position, n=1, max_dist=2500):        
    """
