...
cdef inline void _init_pnode2(IntervalNode node, int id, int start, int end, int value):
    node.id = id
    node.cleft_id = EMPTY
    node.cright_id = EMPTY
    node.croot_id = EMPTY
...
def _init_node(np.ndarray[IntervalNode, ndim=1] tbl, int id, int start, int end, int value):
    _init_pnode2(tbl[id], id, start, end, value)
...
def init_tree2(tbl):
    #cdef np.ndarray[IntervalNode] tbl = np.zeros(num_rows+1, dtype=interval_node_dtype)
